...
test("users can add nodes, wire ports, reject invalid wiring and remove real rules", async ({ page }) => {
	...
	await page.getByRole("tab", { name: "Logic", exact: true }).click();
	await page.getByRole("button", { name: "Add Notify me node" }).click();
	await page.getByRole("button", { name: "Fit graph" }).click();
	await page.getByRole("button", { name: "Focus timer output active", exact: true }).focus();
	...
	await page.getByRole("button", { name: "Focus timer output finished", exact: true }).focus();
	await page.keyboard.press("Enter");
	await page.getByRole("button", { name: "Notify me input finished", exact: true }).focus();
	await page.keyboard.press("Enter");
	await page.getByRole("button", { name: "Apply to routine" }).click();
	...
	await page.getByRole("button", { name: "Focus timer output active", exact: true }).dragTo(page.getByRole("button", { name: "Control apps input gate", exact: true }));
	await page.getByRole("button", { name: "Apply to routine" }).click();
	...
	const heading = page.getByRole("button", { name: "Configure Focus timer", exact: true });
