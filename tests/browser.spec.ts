...
test("edits, adds, reorders, undoes, and persists a tool", async ({ page }) => {
	...
	await page.getByRole("tab", { name: "Routine", exact: true }).click();
	await page.getByLabel("Routine name", { exact: true }).fill("My quiet morning");
	await page.getByLabel("Routine name", { exact: true }).press("Tab");
	...
	await page.getByLabel("Title", { exact: true }).fill("Pages read");
	await page.getByLabel("Title", { exact: true }).press("Tab");
	await expect(page.getByRole("heading", { name: "Pages read" })).toBeVisible();
	await page.getByRole("button", { name: "Move Pages read up", exact: true }).click();
	await expect(page.locator(".block-outline li").nth(3)).toContainText("Pages read");
	await page.getByRole("button", { name: "Undo edit", exact: true }).click();
	await expect(page.locator(".block-outline li").nth(4)).toContainText("Pages read");
...
test("exports strict JSON and cleans dependent rules when deleting blocks", async ({ page }) => {
	...
	await page.goto(STARTER_URL);
	await expect(page.getByText("Saved on this browser")).toBeVisible();
	await page.getByRole("button", { name: "Select Leave distractions outside", exact: true }).click();
	await page.getByRole("button", { name: "Remove block", exact: true }).click();
	const download_event = page.waitForEvent("download");
	await page.getByRole("button", { name: "Export routine", exact: true }).click();
...
test("bad import leaves the library alone; a valid file becomes a new tool", async ({ page }) => {
	await seed_library(page, [starter_document]);
	await page.goto("/");
	await page.getByLabel("Import routine file").setInputFiles({ name: "broken.json", mimeType: "application/json", buffer: Buffer.from("broken") });
	...
	await page.getByLabel("Import routine file").setInputFiles({ name: "new.json", mimeType: "application/json", buffer: Buffer.from(JSON.stringify({ ...starter_document, name: "Imported space" })) });
...
test("layout reflows without document overflow", async ({ page }) => {
	...
	for (const width of [1440, 768, 375]) {
		...
		await page.goto("/");
		await expect(page.getByRole("heading", { name: "My routines", exact: true })).toBeVisible();
