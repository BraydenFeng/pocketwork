...
test("edits, adds, reorders, undoes, and persists a tool", async ({ page }) => {
	...
	await page.getByLabel("App name", { exact: true }).fill("My quiet morning");
...
test("exports strict JSON and cleans dependent rules when deleting blocks", async ({ page }) => {
	...
	await page.getByRole("button", { name: "Export tool", exact: true }).click();
...
test("bad import leaves the library alone; a valid file becomes a new tool", async ({ page }) => {
	await seed_library(page, [starter_document]);
	await page.goto("/");
	await page.getByLabel("Import tool file").setInputFiles({ name: "broken.json", mimeType: "application/json", buffer: Buffer.from("broken") });
	...
	await page.getByLabel("Import tool file").setInputFiles({ name: "new.json", mimeType: "application/json", buffer: Buffer.from(JSON.stringify({ ...starter_document, name: "Imported space" })) });
