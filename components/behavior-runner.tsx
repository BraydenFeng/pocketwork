...
export function BehaviorRunner({ graph, simple = false }: { graph: LogicGraph; simple?: boolean }) {
	...
	return <section className="behavior-test"><SectionLabel>{simple ? "YOUR CONNECTED BLOCKS" : "TEST BEHAVIORS"}</SectionLabel><p className="supporting">Simulated location and app usage. Test progress stays here; no phone restrictions or notifications are changed.</p>
		<details className="preview-test-options" open={simple ? undefined : true}><summary>Simulate time, location, and app usage</summary>
		...
		</details><div className="behavior-controls">{graph.nodes.filter(n => n.kind === "button" || n.kind === "check_in").map(n => <Button key={n.id} onClick={() => run(n.id)}>{n.config?.label || n.kind}</Button>)}</div>
		...
		<BuilderControls nodes={behavior_part(graph).nodes} state={state} outputs={outputs} input={(id,value) => run(undefined, location, Date.now()+offset, { inputs: { [id]: value } })} submit={(node,values) => run(undefined, location, Date.now()+offset, { submission: { node, values } })} timer={timer_command => run(undefined, location, Date.now()+offset, { timer_command })} />
		<details className="preview-test-options" open={simple ? undefined : true}><summary>Connection details</summary><dl>{graph.nodes.filter(n => is_behavior(n.kind)).map(n => <div key={n.id}><dt>{n.config?.label || n.kind}</dt><dd>{Object.entries(outputs[n.id] ?? {}).map(([port,s]) => `${port}: ${s.available === false ? "Unavailable" : typeof s.value === "object" ? Array.isArray(s.value) ? `${s.value.length} entries` : "Record" : String(s.value)}`).join(" · ") || "Ready"}</dd></div>)}</dl></details>
