...
function BehaviorSettings({ node, update }: { node: LogicNode; update: (node: LogicNode) => void }) {
	...
	return <>
		...
		{["location", "arrive", "leave"].includes(node.kind) && <p>Uses the saved location on the phone. Location changes are observed while this routine is open. Initial unknown location does not count as an arrival.</p>}
