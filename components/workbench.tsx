...
import { ArrowDown, ArrowLeft, ArrowUp, ArrowUpRight, Bell, BookOpen, Braces, Check, ChevronRight, Download, FileText, GripVertical, Hash, Info, Layers2, LayoutTemplate, ListChecks, MonitorSmartphone, MousePointer2, Play, Plus, Redo2, RotateCcw, Search, Shield, SlidersHorizontal, Smartphone, Timer, Trash2, Type, Undo2, Workflow, X } from "lucide-react";
...
export function Workbench({ tool, on_save, on_back, storage_blocked, storage_error, on_replace_unreadable, on_dismiss_error }: {
	...
	function add_block(type: BlockType) {
		if (document.blocks.length >= 20 || ((type === "timer" || type === "screen_time") && document.blocks.some((block) => block.type === type))) { return; }
		const block = create_block(type); commit({ ...document, blocks: [...document.blocks, block] }); select_block(block.id); set_view("canvas");
	}
	...
	function rules_panel() {
		return <><div className="behavior-intro"><Workflow /><div><h2>What happens when you focus?</h2><p>Turn these rules on or off, then use Try it to test them. Rules control behavior; blocks control what appears on your screen.</p><Button variant="quiet" onClick={() => open_preview(true)}><Play />Test these rules</Button></div></div>
			<div className="recipe"><SectionLabel number="WHEN">A focus session starts</SectionLabel><div className="recipe-action"><Shield /><div><strong>Keep selected apps out of the way</strong><p>Choose apps privately on your iPhone. They are released when the session ends or you stop it.</p></div></div><Toggle label="Block during focus" description={has_timer && has_shield ? "Uses your timer + Screen Time block" : "Add both a timer and Screen Time block first"} checked={document.rules.block_during_focus} disabled={!has_timer || !has_shield} on_change={(value) => update_rule("block_during_focus", value)} /></div>
			<div className="recipe"><SectionLabel number="WHEN">The focus timer finishes</SectionLabel><div className="recipe-action"><Bell /><div><strong>A gentle heads-up</strong><p>The iPhone host can schedule a local notification. Ending a session early cancels it.</p></div></div><Toggle label="Notify on completion" description={has_timer ? "Requires notification permission on iPhone" : "Add a timer first"} checked={document.rules.notify_on_complete} disabled={!has_timer} on_change={(value) => update_rule("notify_on_complete", value)} /></div>
		...
		<main className="workbench-grid">
			<aside className="library" aria-label="Component library"><div className="panel-heading"><SectionLabel number="01">ADD BLOCKS</SectionLabel><Layers2 /></div>
				...
				<div className="library-list">{block_catalog.filter((entry) => `${entry.label} ${entry.description}`.toLowerCase().includes(query.toLowerCase())).map((entry) => {
					const included = (entry.type === "timer" || entry.type === "screen_time") ? document.blocks.find((block) => block.type === entry.type) : undefined;
				...
				})}{block_catalog.filter((entry) => `${entry.label} ${entry.description}`.toLowerCase().includes(query.toLowerCase())).length === 0 && <p className="empty-hint">No matching blocks. Try “timer” or “note”.</p>}</div>
				...
				<div className="library-footer"><MonitorSmartphone /><strong>Build here. Test here. Run on iPhone.</strong><p>This browser previews your tool. Running it on iPhone requires the native app, currently in development.</p><button className="text-button" type="button" onClick={() => open_inspector("device")}>What works on iPhone <ArrowUpRight /></button></div>
			</aside>
			...
			<aside ref={inspector_ref} tabIndex={-1} className="inspector" aria-label="Inspector"><div className="panel-heading"><SectionLabel number="02">SETTINGS</SectionLabel><SlidersHorizontal /></div><Button className="return-to-preview" variant="quiet" onClick={() => open_preview(interactive)}><MonitorSmartphone />Back to preview</Button><div className="inspector-tabs" role="tablist" aria-label="Inspector section">{(["block", "app", "device"] as const).map((tab) => <button type="button" role="tab" aria-selected={inspector === tab} key={tab} onClick={() => set_inspector(tab)}>{tab === "block" ? "Block" : tab === "app" ? "App" : "iPhone"}</button>)}</div>
				{inspector === "block" && (selected_block ? <div className="inspector-content" key={selected_block.id}><div className="inspector-title"><span className="block-glyph">{(() => { const Icon = block_catalog.find((entry) => entry.type === selected_block.type)!.icon; return <Icon />; })()}</span><div><h2>{block_catalog.find((entry) => entry.type === selected_block.type)!.label}</h2><span className="supporting">Changes appear in the preview. Text saves when you leave a field.</span></div></div>
					...
					{selected_block.type === "screen_time" && <><div className="capability-note"><Shield /><strong>Your selection stays on your phone.</strong><p>Choose apps using Apple’s private picker in the native host. Website and category selections are supported there too.</p></div><Toggle label="Block during focus" description={has_timer ? "Release when the session ends or is stopped" : "Add a focus timer first"} checked={document.rules.block_during_focus} disabled={!has_timer} on_change={(value) => update_rule("block_during_focus", value)} /><Button onClick={() => set_inspector("device")}><Smartphone />View device requirements</Button></>}
				...
				{inspector === "app" && <div className="inspector-content"><div className="inspector-title"><LayoutTemplate /><h2>Name your tool</h2></div><TextField label="App name" value={document.name} required on_commit={(name) => commit({ ...document, name })} /><TextField label="Description" value={document.description} multiline max_length={200} on_commit={(description) => commit({ ...document, description })} /><div className="capability-note"><strong>Saved with your other tools.</strong><p>Every tool is kept on this browser. Export a file to move one to your iPhone or another computer. There is no cloud sync or account yet.</p></div><Button onClick={on_back}><ArrowLeft />Back to my tools</Button></div>}
				{inspector === "device" && <div className="inspector-content"><div className="inspector-title"><Smartphone /><h2>From canvas to iPhone</h2></div><span className="preview-badge">NATIVE HOST · DEVELOPMENT</span><p className="supporting">The browser is ready to use. The iPhone host source is included, but it is not an App Store app yet.</p><ol className="device-steps"><li><strong>Build the native host</strong><p>Use a Mac with Xcode. Follow ios/README.md to configure signing, App Groups, and Family Controls.</p></li><li><strong>Export this tool</strong><p>Save the JSON file to iCloud Drive or send it to your iPhone.</p></li><li><strong>Import and grant access</strong><p>Open the host, import from Files, then select the apps to block and allow notifications if desired.</p></li></ol><div className="capability-note"><Info /><strong>Honest permission boundaries</strong><p>Nothing here has Apple approval yet. Real blocking needs signing and device verification. The user can always end a session.</p></div><p className="supporting">Calendar, location, widgets, MCP, and subscription billing are not part of this build.</p></div>}
