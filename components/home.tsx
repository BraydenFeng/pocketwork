		...
		<main className="home-main">
			<section className="home-section" id="my-tools" aria-labelledby="my-tools-heading">
				...
				{tools.length ? <ul className="tool-grid" aria-label="Your routines">{tools.map((entry) => {
					...
					<button type="button" className="tool-open" onClick={() => on_open(entry.document.id)} aria-label={`Open ${entry.document.name}`}><strong>{entry.document.name}</strong><span className="tool-summary">{summarize_tool(entry.document)}</span>{entry.document.description && <span className="supporting">{entry.document.description}</span>}<span className="tool-meta">{entry.document.home_allowance ? (entry.document.enabled ? "Home allowance enabled" : "Home allowance off") : schedule?.type === "schedule" ? describe_status(schedule, entry.document.enabled === true, now) : format_edited(entry.updated_at, now)}</span></button>
