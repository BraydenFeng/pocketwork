...
export const home_policy_schema = z.object({
	...
	outside_windows: z.literal("block_at_home"),
...
export const requested_home_policy: HomePolicy = {
	timezone: "America/Los_Angeles", away_usage_counts: false, outside_windows: "block_at_home",
	rules: [
...
export function home_decision(policy: HomePolicy, day: number, time: string, at_home: boolean, consumed: number) {
	...
	return { count_usage: at_home && allowed && consumed < (rule?.allowance_minutes ?? 0), blocked: at_home && (!allowed || consumed >= (rule?.allowance_minutes ?? 0)), remaining: Math.max(0, (rule?.allowance_minutes ?? 0) - consumed) };
}
