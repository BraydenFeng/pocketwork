...
export type NodeKind = "timer" | "schedule" | "apps" | "notification" | "home" | "usage" | "allowance";
export type LogicNode = { id: string; kind: NodeKind; x: number; y: number; block?: Block; policy?: HomePolicy };
...
export const node_catalog: Record<NodeKind, { title: string; detail: string; inputs: string[]; outputs: string[] }> = {
	timer: { title: "Focus timer", detail: "Starts when you press Start", inputs: [], outputs: ["active", "finished"] },
	schedule: { title: "Time window", detail: "Days and hours that repeat", inputs: [], outputs: ["active", "outside"] },
	home: { title: "At home", detail: "Your iPhone's saved home", inputs: [], outputs: ["present"] },
	...
	notification: { title: "Notify me", detail: "When a focus timer finishes", inputs: ["finished"], outputs: [] },
};
export const graph_schema = z.object({
	nodes: z.array(z.object({ id: z.string().regex(/^[a-zA-Z0-9_-]{1,64}$/), kind: z.enum(["timer", "schedule", "apps", "notification", "home", "usage", "allowance"]), x: z.number().finite(), y: z.number().finite(), block: block_schema.optional(), policy: home_policy_schema.optional() }).strict()).max(7),
	connections: z.array(z.object({ from: z.string(), output: z.string(), to: z.string(), input: z.string() }).strict()).max(16),
}).strict();
...
export function connect(graph: LogicGraph, edge: Connection): LogicGraph {
	...
	if (!supported.has(`${from.kind}.${edge.output}>${to.kind}.${edge.input}`)) { throw new Error("These ports do not match. Choose a compatible input."); }
	...
	return { ...graph, connections: [...graph.connections, edge] };
}
...
export function compile_graph(base: AppDocument, graph: LogicGraph): AppDocument {
	if (graph.nodes.length > 7 || new Set(graph.nodes.map((node) => node.id)).size !== graph.nodes.length) { throw new Error("Use unique node IDs and at most seven logic nodes."); }
	const by_kind = (kind: NodeKind) => graph.nodes.find((node) => node.kind === kind);
	for (const kind of Object.keys(node_catalog) as NodeKind[]) { if (graph.nodes.filter((node) => node.kind === kind).length > 1) { throw new Error(`This iPhone runtime supports one ${node_catalog[kind].title.toLowerCase()} per routine.`); } }
